...
const TOOLTIP_STYLE = { borderRadius: "12px", border: "1px solid #e2e8f0", fontSize: "12px" };
...
export default function AnalyticsDashboard({
  ...
  const stats = [
    ...
    { label: "총 영업이익",     value: formatKRW(totalOp),     sub: "전체 합산",  color: totalOp  >= 0 ? "text-emerald-600" : "text-red-500" },
    { label: "평균 영업이익률", value: `${avgMargin.toFixed(1)}%`, sub: "전체 평균", color: avgMargin >= 0 ? "text-emerald-600" : "text-red-500" },
  ];

  if (reports.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-slate-200 bg-white px-6 py-20 text-center">
        ...
        <Link href="/upload" className="mt-6 rounded-2xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-700">
          첫 파일 업로드하기
        </Link>
      </div>
    );
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-6 py-4">
          <div className="flex items-center gap-3">
            <h2 className="text-sm font-semibold text-slate-800">재무 분석</h2>
            {/* 리포트 선택 */}
            <select
              value={selectedId}
              onChange={(e) => handleReportChange(e.target.value)}
              disabled={loading}
              className="rounded-xl border border-slate-200 bg-slate-50 py-1.5 pl-3 pr-7 text-xs text-slate-600 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300 disabled:opacity-50"
            >
              {reports.map((r) => (
            ...
            {([
              ...
              <button
                ...
                className={`rounded-[10px] px-3 py-1.5 text-xs font-medium transition ${
                  chartTab === t.value ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"
                }`}
              >
                {t.label}
              </button>
          ...
          {!hasChartData ? (
            ...
            <>
              ...
              {chartTab === "bar" && (
                <>
                  <div className="mb-5 flex items-center justify-between">
                    <p className="text-xs text-slate-500">매출 · 매출원가 · 판관비</p>
                    <div className="flex rounded-xl bg-slate-100 p-0.5">
                      {([
                        ...
                        <button
                          ...
                          className={`rounded-[10px] px-3 py-1.5 text-xs font-medium transition ${
                            barPeriod === p.value ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"
                          }`}
                        >
                          {p.label}
                        </button>
                  ...
                  <ResponsiveContainer width="100%" height={260}>
                    <BarChart data={barData} barCategoryGap="30%" barGap={3}>
                      ...
                      <XAxis dataKey="month" tick={AXIS_TICK} {...shared} />
                      <YAxis
                        ...
                        {...shared}
                      />
                      <Tooltip formatter={tooltipFmt} contentStyle={TOOLTIP_STYLE} />
              ...
              {chartTab === "pie" && (
                <>
                  <div className="mb-5">
                    <p className="text-xs text-slate-500">전체 매출 중 계정과목별 비율</p>
                  </div>
                  {pieData.length === 0 ? (
                    <div className="flex items-center justify-center py-12 text-sm text-slate-400">
                      매출 데이터가 없습니다.
                        ...
                        {pieData.map((d, i) => {
                          ...
                          return (
                            <div key={d.name} className="flex items-center gap-2">
                              ...
                              />
                              <span className="min-w-0 flex-1 truncate text-xs text-slate-600">{d.name}</span>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between px-5 pt-5">
          ...
          <Link href="/dashboard/reports" className="text-xs font-semibold text-blue-600 transition hover:text-blue-700">
            전체 보기 →
          </Link>
        </div>
            ...
            <li key={r.id}>
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-xs text-slate-400">{formatDate(r.created_at)} · {r.row_count}건</p>
                </div>
                <div className="ml-4 shrink-0 text-right">
                  <p className="text-xs text-slate-400">영업이익</p>
                  <p className={`text-sm font-semibold ${r.operating_profit < 0 ? "text-red-500" : "text-emerald-600"}`}>
