...
export default function SignupPage() {
  ...
  async function handleSignup() {
    ...
    if (!name.trim()) {
      setError(accountType === "personal" ? "이름을 입력해주세요." : "회사명을 입력해주세요.");
      return;
    }

    if (!identifier.trim()) {
      setError(accountType === "personal" ? "생년월일을 입력해주세요." : "사업자등록번호를 입력해주세요.");
      return;
    }

    if (accountType === "business" && identifier.replace(/\D/g, "").length !== 10) {
      ...
      options: {
        data: {
          ...
          ...(accountType === "personal"
            ? { birth_date: identifier }
            : { business_number: identifier }),
        },
      },
    });
