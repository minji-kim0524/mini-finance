    ...
    <div className="min-h-screen bg-slate-50 px-4 py-16">
      <div className="mx-auto max-w-2xl space-y-10">
        <div className="text-center space-y-2">
          <h1 className="text-3xl font-bold text-slate-900">플랜 선택</h1>
          <p className="text-slate-500">지금 시작하고 Pro로 무제한 분석하세요.</p>
        </div>

        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {/* Free */}
          <div className="rounded-3xl border border-slate-200 bg-white p-7 space-y-5">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Free</p>
              <p className="mt-2 text-4xl font-bold text-slate-900">₩0</p>
              <p className="mt-1 text-sm text-slate-500">무료로 시작</p>
            </div>
          ...
          {/* Pro */}
          <div className="rounded-3xl border-2 border-blue-500 bg-white p-7 space-y-5 relative">
            ...
            <div>
              ...
              <p className="mt-2 text-4xl font-bold text-slate-900">₩9,900</p>
              <p className="mt-1 text-sm text-slate-500">월 구독</p>
            </div>
            <ul className="space-y-2.5 text-sm text-slate-600">
...
function FeatureItem({ children, accent }: { children: React.ReactNode; accent?: boolean }) {
  return (
    <li className="flex items-center gap-2">
      <svg className={`h-4 w-4 shrink-0 ${accent ? "text-blue-500" : "text-slate-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
